...
const authenticateToken = async (req, res, next) => {
  try {
    ...
    if (!db) {
      console.error('❌ [Auth] Database not initialized in authenticateToken');
...
app.post('/api/auth/register', csrfProtection, async (req, res) => {
  try {
    ...
    // Validate CSRF token (дополнительно к middleware)
    if (!csrfToken || !validateCSRFToken(csrfToken)) {
      return res.status(403).json({ error: 'Invalid CSRF token' });
    }
    ...
    if (error instanceof Error && error.message.includes('UNIQUE constraint failed')) {
      console.error('❌ [Auth] Email already exists:', requestEmail);
...
// API для сохранения истории чата
// GET /api/chat/messages - получить историю чата пользователя
app.get('/api/chat/messages', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    
    if (!userId) {
      console.error('❌ [Chat History] userId is undefined in req.user:', req.user);
      return res.status(400).json({ error: 'User ID not found' });
    }

    const messages = await db.all(
      `SELECT id, role, content, is_audio, created_at 
       FROM chat_messages 
       WHERE user_id = ? 
    ...
    console.log(`✅ [Chat History] Loaded ${messages.length} messages for user ${userId}`);
    console.log(`📋 [Chat History] Messages:`, messages.map(m => ({ id: m.id, role: m.role, content: m.content.substring(0, 50) + '...' })));
    res.json({ messages });
  } catch (error) {
...
// POST /api/chat/messages - сохранить сообщение в историю
app.post('/api/chat/messages', authenticateToken, async (req, res) => {
  try {
    const userId = req.user.id;
    const { role, content, isAudio = false } = req.body;
    ...
    console.log(`💾 [Chat History] Saving message for user ${userId}:`, { role, contentLength: content.length, isAudio });

    const result = await db.run(
      `INSERT INTO chat_messages (user_id, role, content, is_audio, created_at)
       VALUES (?, ?, ?, ?, ?)`,
      [userId, role, content, isAudio ? 1 : 0, new Date().toISOString()]
    );

    console.log(`✅ [Chat History] Saved message for user ${userId}, messageId: ${result.lastID}`);
    res.json({ 
      success: true, 
...
// DELETE /api/chat/messages - очистить историю чата пользователя
app.delete('/api/chat/messages', authenticateToken, async (req, res) => {
  try {
    ...
    console.log(`✅ [Chat History] Cleared history for user ${userId}`);
    res.json({ success: true });
  } catch (error) {
